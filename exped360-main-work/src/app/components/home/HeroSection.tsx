...
export const HeroSection: React.FC = () => {
  ...
  const [slides, setSlides] = useState<{ image: string; title?: string; location?: string }[]>(defaultSlides);
    ...
    apiService
      ...
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) {
          setSlides(data.map((d) => ({ image: d.imageUrl, title: d.altText || undefined })));
        }
      })
      .catch(() => {});
