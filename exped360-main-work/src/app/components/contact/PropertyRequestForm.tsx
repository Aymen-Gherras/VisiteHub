...
export default function PropertyRequestForm({ defaultType = '', defaultWilaya = '' }: PropertyRequestFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const subject = `Demande d'information - ${propertyType || 'Bien immobilier'}`;
      const to = process.env.NEXT_PUBLIC_CONTACT_EMAIL || 'contact@visitehub.com';
      const emailBody = encodeURIComponent(
        `Bonjour,\n\nJe souhaite être contacté(e) au sujet d'un bien.\n\n` +
        `Nom: ${name}\n` +
        `Téléphone: ${phone}\n` +
        `Email: ${email}\n` +
      ...
      apiService.createDemandeWithImages(demandeData, images).catch(console.error);

      window.location.href = `mailto:${to}?subject=${encodeURIComponent(subject)}&body=${emailBody}`;

      alert('Votre message est prêt dans votre client email. Votre demande a également été sauvegardée dans notre système.');
      setName('');
      setPhone('');
      setEmail('');
      setPropertyType('');
      ...
      setError(err instanceof Error ? err.message : 'Erreur lors de la préparation de l\'email');
    } finally {
  ...
  const openWhatsApp = () => {
    const body = encodeURIComponent(
      `Bonjour, je souhaite mettre mon bien en ligne.\nNom: ${name}\nTéléphone: ${phone}\nEmail: ${email}\nType: ${propertyType || '-'}\nWilaya: ${wilaya || '-'}\nIntention: ${propertyIntention === 'sell' ? 'Vendre' : 'Louer'}\n${want360 ? 'Je souhaite une visite 360°.' : ''}\nPapiers: ${selectedPapiers.length > 0 ? selectedPapiers.join(', ') : 'Non spécifié'}\nDescription: ${description}`
    );
...
>
  ...
  <option value="Touggourt">Touggourt</option>
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4">
            ...
            <button type="button" onClick={openWhatsApp} className="flex-1 bg-green-500 text-white py-4 rounded-xl font-semibold shadow-md hover:bg-green-600 transition-all flex items-center justify-center">
