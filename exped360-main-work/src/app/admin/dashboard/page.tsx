...
export default function AdminDashboard() {
  ...
  const revenue = useMemo(() => properties.reduce((sum, p) => sum + (p.price || 0), 0), [properties]);
          ...
          <ul className="divide-y divide-slate-100">
            {properties.slice(0, 8).map((p) => (
              <li key={p.id} className="py-2 flex items-center justify-between">
                ...
                <div className="text-sm font-semibold text-slate-700">{Math.round(p.price).toLocaleString('ar-DZ')} DZD</div>
              </li>
            ))}
            {properties.length === 0 && (
